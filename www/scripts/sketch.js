...
function UDP_send(MESSAGE, PORT, HOST) {
  if (MESSAGE == "1" && time_to_send == true) {
    ...
    client.send(message, 0, message.length, PORT, HOST, function (err, bytes) {
      if (err) throw err;
      console.log(
        " : UDP message " + message + " sent to " + HOST + ":" + PORT
      );
      client.close();
    });
  }
...
function setup() {
  ...
  document.getElementById("configpath").innerHTML =
    localStorage.getItem("config_file_path");
  document.getElementById("configmessage").innerHTML =
    "Only the IP of modules on the jacket can be configured.  <br> Configuration file location : ";
    ...
    for (var l = 1; l < motorGUI.length; l++) {
      ...
      for (var y = 0; y < UDP_motorid.length; y++) {
        if (UDP_motorid[y] != 0 && UDP_motorid[y] < motorGUI.length) {
          motorGUI[UDP_motorid[y]].API_activate(
            delay_times[UDP_motorid[y]],
            intensities[UDP_motorid[y]]
          );
...
function show_message(msg) {
  document.getElementById("messagebox").innerHTML =
    "<i style='font-weight:normal'>" + msg + "</i>";
}
...
function mouseReleased() {
  ...
  if (configuration_mode_on && locked) {
    ...
    if (typeof Storage !== "undefined") {
      ...
      console.log(
        "Saved Location : Motor " +
          current_dragged_module +
          " : (" +
          localStorage.getItem(current_dragged_module + "-x") +
          "," +
          localStorage.getItem(current_dragged_module + "-y") +
          ")"
      );
    ...
    for (k = 1; k <= 20; k++) {
      if (autoID <= number_of_motors) {
        if (localStorage.getItem(autoID + "-x")) {
          motorGUI[autoID] = new VibrationMotor(
            parseFloat(localStorage.getItem(autoID + "-x")),
            parseFloat(localStorage.getItem(autoID + "-y")),
            autoID
          );
        } else {
...
function list_configuration() {
  for (var l = 1; l <= localStorage.getItem("MaxID"); l++) {
    if (
      localStorage.getItem(l + "-x") != null ||
      localStorage.getItem(l + "-y") != null
    ) {
      console.log(
        "Motor " +
          l +
          " : (" +
          localStorage.getItem(l + "-x") +
          "," +
          localStorage.getItem(l + "-y") +
          ")"
      );
    }
  }
...
function updatedata(NAME) {
  ...
  stream.once("open", function (fd) {
    ...
    for (var l = 1; l <= localStorage.getItem("MaxID"); l++) {
      if (
        localStorage.getItem(l + "-IP") != null ||
        localStorage.getItem(l + "-port") != null
      ) {
        stream.write(
          l +
            "," +
            localStorage.getItem(l + "-IP") +
            "," +
            localStorage.getItem(l + "-port") +
            "\n"
        );
      }
    }
    ...
    if (!configuration_mode_on) {
      ...
      if (this.is_vibrating && this.init_y <= box_boundary_y_coordinate) {
        ...
        if (
          ...
          UDP_send(
            "1",
            localStorage.getItem(this.ID + "-port"),
            localStorage.getItem(this.ID + "-IP")
          );
        ...
        this.y = this.init_y;
        this.x += random(
          -this.max_pixel_vibration_animation,
          this.max_pixel_vibration_animation
        );
        this.y += random(
          -this.max_pixel_vibration_animation,
